...
const SAMPLE_PASTE = `Day 1 - Arrive Chicago, deep dish pizza at Lou Malnati's
Day 2 - Architecture boat tour, Millennium Park, Navy Pier
Day 3 - Drive to Milwaukee, Third Ward, lakefront brewery tour
Day 4 - Milwaukee Art Museum, cheese curds, drive to Madison
Day 5 - State Street, Capitol Square, Dane County Farmers' Market
Day 6 - Drive to Minneapolis, Mill City Museum, North Loop dining
Day 7 - Chain of Lakes, fly home`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Midwest itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <PathCard
        ...
        desc="Popular routes you can customize — Great Lakes, Plains, Ohio Valley & more"
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "Used this on our Great Lakes road trip and it had everything — directions between cities, restaurant picks, even local tips. Felt like traveling with a friend who knows the Midwest."
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.oceanTeal, marginTop: 8 }}>
          — Sarah & Mike, 7-day Great Lakes trip
        </div>
      </div>
    </>
  );
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'great-lakes') return t.route.match(/Chicago|Milwaukee|Detroit|Cleveland/i);
          if (filter === 'plains') return t.route.match(/Kansas City|Minneapolis|Omaha|Des Moines|Madison/i);
          if (filter === 'ohio-valley') return t.route.match(/Cincinnati|Indianapolis|Columbus|St\.?\s*Louis/i);
          return true;
        });
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto' as const }}>
          {['all', 'great-lakes', 'plains', 'ohio-valley'].map((f) => (
            <Pill key={f} active={filter === f} onClick={() => setFilter(f)}>
              {f === 'great-lakes' ? 'Great Lakes' : f === 'ohio-valley' ? 'Ohio Valley' : f.charAt(0).toUpperCase() + f.slice(1)}
            </Pill>
          ))}
        </div>
    ...
    <>
                  ...
                  <div key={item.id} style={{
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: COLORS.oceanTeal, fontWeight: 600 }}>
                        {item.type === 'transport' ? 'Directions included' : item.type === 'food' ? 'Local pick' : 'Local tips included'}
                      </div>
                    </div>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not just Google Maps pins — actual driving directions, parking tips, and transit options between cities" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '🍕', title: 'Local Food Picks', desc: 'Curated restaurant recommendations for every stop — from deep dish to BBQ to cheese curds' },
            { icon: '📞', title: 'Key Contacts', desc: 'Emergency numbers, roadside assistance, and useful contacts for your route' },
            { icon: '🎒', title: 'Smart Packing', desc: 'Checklist auto-generated from your activities — brewery tour adds layers for the cold' },
            { icon: '💰', title: 'Budget Tracker', desc: 'Log expenses, see typical costs, track your spending — all offline' },
            { icon: '💵', title: 'Tipping Guide', desc: 'Standard tipping for every Midwest situation, from restaurants to rideshares' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '📶', title: 'Works Offline', desc: 'Everything cached to your phone. Dead zone on the highway? No problem' },
          ].map((f, i) => (
            <div key={i} style={{
