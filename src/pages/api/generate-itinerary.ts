...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'food': ['chicago', 'kansas-city', 'cincinnati'],
    'bbq': ['kansas-city', 'st-louis'],
    'beer': ['milwaukee', 'chicago', 'minneapolis'],
    'sports': ['chicago', 'detroit', 'cleveland'],
    'music': ['chicago', 'detroit', 'st-louis'],
    'culture': ['chicago', 'minneapolis', 'detroit'],
    'history': ['detroit', 'chicago', 'indianapolis'],
    'nature': ['minneapolis', 'madison', 'cleveland'],
    'family': ['chicago', 'indianapolis', 'milwaukee'],
    'budget': ['des-moines', 'omaha', 'madison'],
    'luxury': ['chicago', 'minneapolis', 'kansas-city'],
    'great-lakes': ['chicago', 'milwaukee', 'detroit', 'cleveland'],
    'plains': ['kansas-city', 'minneapolis', 'omaha', 'des-moines', 'madison'],
    'ohio-valley': ['cincinnati', 'indianapolis', 'columbus', 'st-louis'],
    'architecture': ['chicago', 'columbus', 'detroit'],
    'art': ['chicago', 'minneapolis', 'columbus'],
  };
  ...
  return ['chicago', 'milwaukee', 'minneapolis'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Midwest AI Trip Planner. You generate detailed day-by-day itineraries for Midwest USA travel.
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-chicago"
...
- 15+ days: 2-3 items per day, brief descriptions (under 15 words each). Group similar days (e.g. "Days 5-6: Exploring the lakefront in Chicago"). Only include key activities, one meal, and accommodation.
...
{
  "title": "string — trip title like '7-Day Great Lakes Road Trip'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Chicago'",
      "destination": "string — slug like 'chicago'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Millennium Park', 'Art Institute of Chicago', 'Navy Pier'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Midwest USA trip:\n${parts.join('\n')}`;
